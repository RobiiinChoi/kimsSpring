...
//    엔티티가 변해도 API 스펙이 변하지 않는다.
    // **** 매우 중요 : 실무에서는 엔티티를 API 스펙에 노출하면 안됨
    ...
    // 수정 API
    // 회원 수정 API에서 회원 정보를 부분 업데이트 한다. PUT을 사용했지만, PUT은 전체 업데이트를 할 때 사용하는 것이 맞고,
    // 부분 업데이트를 하기위해서는 PATCH나 POST를 사용하는 것이 REST에 맞다
    @PutMapping("/api/v2/memebrs/{id}")
    ...
    static class UpdateMemberResponse {
        ...
        필요하지만, 어떤 API는 name 필드가 필요없을 수 있다.
        ***** 결론적으로 엔티티 대신에 API 스펙에 맞는 별도의 DTO를 노출해야 한다.*/
        // DTO를 최대한 세분화 해서 써야된다는 점이 바로 이부분 때문!
        // 그렇지 않으면 응답 스펙을 맞추기 위해서 로직이 추가되는데 (@JsonIgnore, 별도 뷰 로직)
